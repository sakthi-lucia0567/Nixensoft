...
import { ArrowLeft, Plus, Trash2, Save, GripVertical, Edit } from "lucide-react"
...
const colorOptions = [
  { name: "Blue", bg: "bg-blue-50", hover: "text-blue-600", active: "text-blue-800" },
  { name: "Purple", bg: "bg-purple-50", hover: "text-purple-600", active: "text-purple-800" },
  { name: "Teal", bg: "bg-teal-50", hover: "text-teal-600", active: "text-teal-800" },
  { name: "Green", bg: "bg-green-50", hover: "text-green-600", active: "text-green-800" },
  { name: "Red", bg: "bg-red-50", hover: "text-red-600", active: "text-red-800" },
  { name: "Amber", bg: "bg-amber-50", hover: "text-amber-600", active: "text-amber-800" },
  { name: "Pink", bg: "bg-pink-50", hover: "text-pink-600", active: "text-pink-800" },
  { name: "Indigo", bg: "bg-indigo-50", hover: "text-indigo-600", active: "text-indigo-800" },
]

export default function ServicesEditor() {
  const router = useRouter()
  const { services, updateService, addService, deleteService, reorderServices } = useCMSStore()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleAddNewService} className="bg-blue-600 hover:bg-blue-700">
        ...
        <CardContent>
          <div className="space-y-4">
            {services.map((service) => (
              <div key={service.id} className="flex items-center gap-4 p-4 border rounded-md bg-gray-50">
                <div className="flex-shrink-0">
                  <GripVertical className="h-5 w-5 text-gray-400" />
                </div>
                <div className="flex-grow">
                  <h3 className="font-medium">{service.title}</h3>
                  <p className="text-sm text-gray-500">{service.items.length} items</p>
                </div>
            ...
            {services.length === 0 && (
              <div className="text-center py-8">
                ...
                <Button onClick={handleAddNewService}>Add Your First Service</Button>
              </div>
            )}
          </div>
        </CardContent>
                  ...
                  {colorOptions.map((color) => (
                    <Button
                      ...
                      className={`h-10 ${color.bg} hover:${color.hover} ${
                        editingService.bgColor === color.bg ? "ring-2 ring-offset-2 ring-blue-500" : ""
                      }`}
                ...
                {editingService.items.map((item: any, index: number) => (
                  <div key={index} className="flex items-center gap-2 p-2 border rounded bg-gray-50">
                    <div className="grid grid-cols-2 gap-2 flex-grow">
                      ...
                      <Input
                        value={item.href}
                        onChange={(e) => handleServiceItemChange(index, "href", e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) => handleNewServiceItemChange("href", e.target.value)}
                      placeholder="Link (e.g., /services/web-design)"
                    />
                    <Button type="button" onClick={handleAddServiceItem} size="icon">
